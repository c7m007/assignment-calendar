...
async function loadClasses() {
    try {
        // Fetch columns B through E (B=name, C=full name, D=old color spot, E=hex color)
        ...
        for (const row of data) {
            const name = row[0]?.trim(); // Column B - class name
            const hexColor = row[3]?.trim(); // Column E - hex color (index 3 when fetching B:E)
            
            // Only include rows that:
            // 1. Have a class name
            // 2. Have a valid hex color starting with #
            // 3. Are not header/label rows
            if (name && 
                hexColor && 
                hexColor.startsWith('#') &&
                !name.toUpperCase().includes('SCHEDULE') &&
                !name.toUpperCase().includes('PROFESSOR') &&
