...
async function loadClasses() {
    try {
        // Fetch columns B through D (B=name, C=full name, D=hex color)
        ...
        for (const row of data) {
            const name = row[0]?.trim(); // Column B - class name (index 0)
            const hexColor = row[2]?.trim(); // Column D - hex color (index 2)
            
            console.log('Processing row:', name, hexColor);
