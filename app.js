...
async function loadClasses() {
    try {
        // Fetch columns A and B from Calendar Classes sheet (A=name, B=hex color)
        const data = await fetchSheetData(CONFIG.CLASSES_TAB, 'A1:B50');
        
        console.log('Raw class data:', data);
        
        classes = [];
        
        for (const row of data) {
            const name = row[0]?.trim(); // Column A - class name
            const hexColor = row[1]?.trim(); // Column B - hex color
            
            console.log('Processing class:', name, hexColor);
            
            // Only include rows that have both a class name and a valid hex color
            if (name && 
                name.length > 0 &&
                hexColor && 
                hexColor.startsWith('#')) {
        ...
        console.log('Final loaded classes:', classes);
        return classes;
    } catch (error) {
...
function renderDayAssignments(dayAssignments, isWeekView = false) {
    ...
    // Use default font size for all assignments (no shrinking)
    const fontSize = settings.defaultFontSize;
    ...
    return dayAssignments.map(a => {
        ...
        // Completed items get grey styling, otherwise use class color
